#!/bin/bash
###------------------------------------------------------------------------------------------------
# alias:   aws_deploy.utility.install
# script:  install.sh
# purpose: Installs AWS Deploy Functionality
# version: 1.0.0
###------------------------------------------------------------------------------------------------

###------------------------------------------------------------------------------------------------
## Config
declare -r SELF_IDENTITY="install_awsdeploy"
declare -r SELF_IDENTITY_H="Install AWS Deploy"
...
if(! is_empty "${DIRECTORY_ALIAS_LINKS}"); then
    if [ -d "${DIRECTORY_ALIAS_LINKS}" ]; then
        ...
        for TMP_FILE in $(eval "find ${DIRECTORY_BASE}/scripts -type f 2>/dev/null"); do
            TMP_ALIAS=""
