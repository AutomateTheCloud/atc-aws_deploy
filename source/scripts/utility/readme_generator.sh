#!/bin/bash
###------------------------------------------------------------------------------------------------
# alias:   aws_deploy.utility.readme_generator
# script:  readme_generator.sh
# purpose: Scans AWS Deploy application directory and generates an appropriate README.md
# version: 1.0.0
###------------------------------------------------------------------------------------------------
...
REQUIRED_SOURCE_FILES+=("${LIB_REFERENCE_DETAILS}")
...
if (is_empty "${FILE_README}"); then
    FILE_README="${DISTRIBUTION_README}"
fi
